package com.mejesticpay.eventstore.model;
...
public class PaymentWrapper
{
    ...
    public void updatePayment(InFlightTransactionInfo inFlightTransactionInfo)
    {
        state = inFlightTransactionInfo.getState();
        status = inFlightTransactionInfo.getStatus();
        station = inFlightTransactionInfo.getCurrentStation();
        track = inFlightTransactionInfo.getTrack();
        version = inFlightTransactionInfo.getVersion();
        last_updated_time = Instant.now();
//        last_updated_by = payment.getLastUpdatedBy();
//        created_by = payment.getCreatedBy();
//        source = inFlightTransactionInfo.getSource();
        branch = inFlightTransactionInfo.getBranch();

    }

    public void updatePayment(String serviceType, ServiceData serviceData)
    {
        try
        {

            if("DebitEnrichment".equalsIgnoreCase(serviceType))
                json_debit_enrich = JSONHelper.convertToStringFromObject(serviceData);
//            if(payment.getCreditEnrichment() != null)
//                json_credit_enrich = JSONHelper.convertToStringFromObject(payment.getCreditEnrichment());
//            if(payment.getFraudCheckInfo() != null)
//                json_fraud_check = JSONHelper.convertToStringFromObject(payment.getFraudCheckInfo());
